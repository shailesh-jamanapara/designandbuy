...
class Customer_Contacts_Model Extends Designandbuy_Model
{
    public $model;
    ...
    public function __construct($model = null){
     ...
     private function setTableProperties(){
		 ...
		 'fields' => array(
			 ...
			 'landline_no'=> array(
					 'caption' => array('phone no', ' phone no', 'phone no'),		
					 'type'=>'varchar',
					 'len' => 15,
					  'value' => null,
			  ...
			  'extension'=> array(
					 'caption' => array(' extension', ' extension', ' extension'),		
					 'type'=>'int',
					 'len' => 4,
					  'value' => null,
					   'rules' => array('is_required'=>false, 'regex' => array('server' => '/[^a-zA-Z.\ ]', 'client' => '^[0-9]+$', 'message' => 'Allows only Numbers')),
	...
	*/ 
	public function getAllWithKeyValue( $obArr=array() ) 
	{
		...
		if (!empty($rows) && count($rows) > 0) {
			foreach ($rows as $row) {
				$rowArr[$row['id']]= $row['customer_name'];  
			}
		}
		return $rowArr;
	}
