  ...
  <!-- Product view -->
      <div class="row no-gutters">
          <div class="col-lg-3 col-md-3 mt-1 mb-1 pl-1">
              <span class="font-weight-lighter text-white pull-left mt-2">Back side </span>  
          </div>
          <?php if(isset($request['task']) && $request['task'] == 'edit_template'){?>
            ...
            <?php }else{ ?>
              <div class="col-lg-9 col-md-9 mt-1 mb-1 pr-2 pt-1">
                <button class="mb-1 btn  btn-outline-white bg-hover-orange btn-rounded border-w-2  pull-right  font-weight-lighter"  id="btn_save"> Save & Continue <i class="fa fa-save"></i></button> 
              </div>
            <?php } ?>
      </div>
                                              ...
                                              <div class="featured_hori_block mb-1  border-w-1">
                                                  <div class="featured_hori_thumb_block">
                                                      ...
                                                      <div class="featured_hori_back">
                                                          <div>
                                                              <button class="btn  btn-outline-white bg-hover-orange btn-rounded border-w-1 font-weight-lighter pt-1 pb-1 vect-bg-studio-dark" onclick="javascript:pickAndSetImage('<?php echo base_url();?>/uploads/templates/<?php echo $tmpl['template_image_path_back'];?>');">Use this</button>
                                                          </div>
                                                      </div>
                                                  </div>
                                              </div>
    ...
    <div class="modal-header bg-orange px-3 py-4">
        ...
        <div class="modal-body mt-1">
            <div class="row">
                ...
                <div class="col-lg-6 text-center mt-1  file-uploader-left"   id="my_images_grid">
                <span class="mb-2">
                            Your images
                          </span>
                <div class="card-columns mt-2 " >
                  <div class="card">
                    <img class="card-img-top img-fluid" src="<?php echo asset_url(); ?>img/backgrounds/picjumbo.com_P1010062.jpg" alt="Card image cap">
                  </div>
                  <div class="card">
                    <img class="card-img-top img-fluid" src="<?php echo asset_url(); ?>img/backgrounds/picjumbo.com_P1010057.jpg" alt="Card image cap">
                  </div>
                  <div class="card">
                    <img class="card-img img-fluid" src="<?php echo asset_url(); ?>img/backgrounds/picjumbo.com_P1010041.jpg" alt="Card image">
                  </div>
                  <div class="card">
                    <img class="card-img-top img-fluid" src="<?php echo asset_url(); ?>img/backgrounds/picjumbo.com_P1010062.jpg" alt="Card image cap">
                  </div>
                ...
                <div class="col-lg-6 text-center file-uploader-right mt-1">
                <span class="mb-2">
                            Upload  from your PC
                          </span>
...
if(localStorage.getItem('fields')){
  fields = $.parseJSON(localStorage.getItem('fields'));
  $.each(fields, function(idx, elem) {
    $('button#'+idx).parent().remove();
  });
}
var used_labels = [];
...
window.addPhotoObject = function(photo_obj_id, obj_shape, obj_name){
  ...
  if(obj_shape == 'square'){
    ...
    fabric.Image.fromURL(url, function(img) {
      var oImg = img.set({ id:newID, width: img.width, height: img.height,  selectable:true, left: 44,	top: 44, stroke: _fcolor, strokeWidth:24, hoverCursor: "pointer", id:newID, objectCaching: false }).scale(0.50);
    canvas.add(oImg);
    canvas.renderAll();
    oImg.set('id', newID);
  ...
  if(obj_shape == 'rounded'){
    fabric.Image.fromURL(url, function(img) {
      var oImg = img.set({ objectCaching: false,id:newID, width: img.width, height: img.height,  selectable:true, left: 48,	top: 48, hoverCursor: "pointer", id:newID, clipTo: roundedCorners.bind(img), stroke:  _fcolor, strokeWidth: 36}).scale(0.50);
    canvas.add(oImg);
    canvas.renderAll();
    oImg.set('id', newID);
    fields[newID] = oImg;
  ...
  if(obj_shape == 'circle'){
    fabric.Image.fromURL(url, function(img) {
      var oImg = img.set({  objectCaching: false, id:newID,  width: img.width, height: img.height,  selectable:true, left: 24,	top: 24, stroke:  _fcolor, hoverCursor: "pointer", id:newID, clipTo: fullRounded.bind(img) }).scale(0.66);
    canvas.add(oImg);
    canvas.renderAll();
    oImg.set('id', newID);
    fields[newID] = oImg;
...
btnEl.onclick = function() {
    ...
    $.ajax({
        url: url,
        data : {'canvas': data_to_pass, to_svg:to_svg, variables: JSON.stringify(variables), fields:JSON.stringify(fields), customer_templates_id: customer_templates_id},
        type:'POST',
        dataType:'json',
...
canvas.on('object:moving', function (e) {
  ...
  //console.log('On moving from left', obj.left);
  //console.log('On moving from top', obj.top);
