...
class Users_Addresses Extends Designandbuy_Controller {
    ...
    private function initLoad() {
        ...
        $this->{$this->model_name}->table_properties['cross_fields']['first_name'] = array(
            'caption' => array('First Name', 'First Name', 'First name  of Employee'),
            'type' => 'varchar',
            'len' => 100,
            'value' => null,
            'in_select' => 1,
        ...
        $this->{$this->model_name}->table_properties['join_tables'][] = array('users', 'users.id = ' . $this->_model . '.user_id AND ' . $this->_model . '.address !=" "  ', 'left');
