...
class User_Login extends Vect_Controller {
	...
	public function __construct(){
		...
		if($this->session->userdata('target_url')){			
			 $this->target_url = $this->session->userdata('target_url');
		}	
	}
	...
	public function index(){	
		...
		if(!$this->session->userdata('loggedin')){			
			...
			$this->load->view('user_login',  $this->web);
	...
	public function create_account(){		
		...
		if ( isset($this->web['request']['task']) && $this->web['request']['task'] == 'create_account' ) {
			...
			if($id > 0 ){
				...
				$customer_name = $this->web['request']['customer_name'];
				...
				$this->sendSignUpEmail( $customer_name, $contact_name, $email, $username, $password, $token);
	...
	public function loginValidate(){
		...
		if(!empty($row) && isset($row[0])){
			...
			*/
			...
			if($row[0]['role_id'] !== 1){
				$customer_id = $this->session->userdata['loggedin']['id'];
				if(isset($this->request['source']) && $this->request['source'] == 'product_info' ){
					$return['status'] = 'success';
					$return['message'] = "User logged in  successfully";
					$return['state'] = "200";
					$return['encoded_id'] = base64_encode($customer_id);
					echo json_encode($return);
					exit;
				}else{
					if( isset($this->target_url) )
							redirect(base_url().'index.php/Studio/template_act?id='.$customer_id );
					else
							redirect(base_url().'index.php/My_Pages/myaccount?id='.$customer_id );
				}
			}
		}else{
			redirect(base_url());
	...
	public function forgot_password(){
		...
		if(isset($this->request['task']) && $this->request['task'] === 'forgetpassword' && isset($this->request['username']) ){
			$this->load->model('users_model');
			$this->users_model->ci_where = array();
			$this->users_model->ci_where["users.username"] = $this->request['username'];
			$row = $this->users_model->getAll();
			if( !empty($row) && count($row) === 1 ){
				...
				$record = $this->customers_model->getById($customer_id);
				...
				if( !empty($record) ){
					...
					$token = '';
					foreach (array_rand($seed, 20) as $k) $token .= $seed[$k];
					$data['token'] = base64_encode($token.'*****'.$username);
					$this->users_model->ci_where = array();
					$this->users_model->save_record($users_id , $data);
					
					if( $this->sendForgotPasswordEmail( $customer_name, $email, $username, $data['token'])){
						$this->load->view('layout/front/header',  $this->web);
						$this->load->view('layout/front/main_menu',  $this->web);
						$this->load->view('forgot_password_link_sent',  $this->web);
						$this->load->view('layout/front/footer',  $this->web);
					}
				}
			}
			...
			if(!$this->session->userdata('loggedin')){			
				$this->load->view('layout/front/header',  $this->web);
				$this->load->view('layout/front/main_menu',  $this->web);
				$this->load->view('forgot_password',  $this->web);
				$this->load->view('layout/front/footer',  $this->web);
			}
		}
	}
	...
	public function reset_password(){
		...
		if( isset($this->request['task']) && $this->request['task'] === 'resetpassword' && isset($this->request['token']) && isset($this->request['password']) ){
			$token = $this->request['token'];
			$this->load->model('users_model');
			$this->users_model->ci_where = array();
			$this->users_model->ci_where["users.token"] = $token;
			$row = $this->users_model->getAll();
			echo  $token;
			print_r($row);
			if( !empty($row) && count($row) === 1 ){
				...
				$data['token'] = '';
				if($this->users_model->save_record($users_id, $data))
					redirect('User_Login');
				else
			...
			if($this->session->userdata('loggedin')){
				redirect('My_Pages/created_templates?');
			}
			if(!$this->session->userdata('loggedin')){
				$this->web['token'] = $this->request['token'];
				$this->load->view('layout/front/header',  $this->web);
				$this->load->view('layout/front/main_menu',  $this->web);
				$this->load->view('reset_password',  $this->web);
				$this->load->view('layout/front/footer',  $this->web);
			}
		}
	}
