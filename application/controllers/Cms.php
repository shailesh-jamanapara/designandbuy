...
class Cms Extends Vect_Controller {
    ...
    public function index() {
	 ...
	 public function aboutus() {
        ...
        $this->load->view('cms/aboutus',  $this->web);
	 ...
	 public function services() {
        ...
        if( isset($this->request['ACTION']) ){
            if( strtolower($this->request['ACTION']) == 'printing'){
                $this->load->view('cms/printing',  $this->web);
            }else{
                $this->load->view('cms/services',  $this->web);
            }
        }else{
            $this->load->view('cms/services',  $this->web);
        }
    ...
    public function printing() {
	 ...
	 public function inquiry() {
		...
		if ( isset($this->web['request']['task']) && $this->web['request']['task'] == 'make_inquiry' ) {
            $data = array();
            $this->load->model('Crm_Contacts_model');
            //$data['crm_contact_name'] =$id ; 
            $data['contact_for'] = $this->web['request']['contact_for'];
            $data['crm_contact_name'] = $this->web['request']['crm_contact_name'];
            $data['contact_email'] = $this->web['request']['contact_email'];
            $data['contact_subject'] = $this->web['request']['contact_subject'];
            $data['contact_number'] = $this->web['request']['contact_number'];
            $data['contact_message'] = $this->web['request']['contact_message'];
            // $data['updated_on'] = date('Y:m:d');
            // $data['updated_by'] = $id;
            // $data['created_at'] = date('Y:m:d');
            // $data['created_by'] = $id;
			$this->Crm_Contacts_model->save_record(0, $data);
				
            // $school_name = $this->web['request']['crm_contact_name'];
            // $contact_name = $this->web['request']['first_name'];
            // $email = $this->web['request']['email'];
            // $username = $data['username'];
            
            print_r( $this->sendInquiryEmail( $data['crm_contact_name'], $data['contact_email'], $data['contact_subject'], $data['contact_number'], $data['contact_message'], $data['contact_for']));
            exit;
       	...
       	$this->load->view('layout/front/header',  $this->web);
        //$this->load->view('layout/front/main_content', $data);
        $this->load->view('cms/inquiry',  $this->web);
		$this->load->view('layout/front/footer',  $this->web);
    }
	 public function contactus() {
       	$this->load->view('layout/front/header',  $this->web);
        //$this->load->view('layout/front/main_content', $data);
        $this->load->view('cms/contactus',  $this->web);
		$this->load->view('layout/front/footer',  $this->web);
    }
