...
class Expense_Types Extends Designandbuy_Controller {
    ...
    public function __construct() {
        ...
        if ($this->load->model(trim($this->model_name))) {
            ...
            $this->web['items'] = ucwords(str_replace('_', ' ', $this->class_name));
            $this->web['item'] = ucwords(str_replace('_', ' ', $this->inflect->singularize($this->class_name)));
        } else {
            return false;
            die();
        }
    ...
    private function initLoad() {
        ...
        $this->web['postdata']['sort_by'] = (isset($this->web['request']['sort_by'])) ? $this->web['request']['sort_by'] : $this->_model . '.expense_type_name';
        $this->web['postdata']['order_type'] = (isset($this->web['request']['order_type'])) ? $this->web['request']['order_type'] : 'ASC';
        $this->web['search'][$this->_model . '.expense_type_name'] = 'Expense Type';
